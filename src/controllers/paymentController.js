...
async function startPaymentController(req, res) {
  try {
    ...
    return res.status(201).json({ transactionId: tx.transactionId, instructions });
  } catch (err) {
...
async function checkoutAndCreatePaymentController(req, res) {
  try {
    ...
    return res.status(201).json({ orderId: result.orderId, transactionId: tx.transactionId, instructions });
  } catch (err) {
